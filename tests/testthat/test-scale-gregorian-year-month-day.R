...
#   df <- data_frame(
#     x = year_month_day(integer(), integer()),
#     y = double()
#   )
#
...
test_that("works with a vector of data", {
  df <- data_frame(
    x = year_month_day(2019, 1) + duration_months(1:5),
    y = 1:5
  )
...
test_that("works with changing the `date_breaks`", {
  df <- data_frame(
    x = year_month_day(2019, 1) + duration_months(1:20),
    y = 1:20
  )
  ...
  p <- ggplot2::ggplot(df, ggplot2::aes(x, y)) +
    ...
    scale_x_year_month_day(
      date_breaks = duration_months(4),
      date_minor_breaks = duration_months(2)
    )
...
test_that("works with changing `date_labels`", {
  df <- data_frame(
    x = year_month_day(2019, 1) + duration_months(1:5),
    y = 1:5
  )
  ...
  p <- ggplot2::ggplot(df, ggplot2::aes(x, y)) +
    ggplot2::geom_line() +
    scale_x_year_month_day(date_labels = "%Y %B")

  vdiffr::expect_doppelganger("date_labels", {
    p
  })
})
...
test_that("can't use invalid `date_breaks` or `date_minor_breaks`", {
  ...
  p <- ggplot2::ggplot(df, ggplot2::aes(x, y)) +
    ggplot2::geom_line() +
    scale_x_year_month_day(date_breaks = duration_days(1))
  ...
  p <- ggplot2::ggplot(df, ggplot2::aes(x, y)) +
    ggplot2::geom_line() +
    scale_x_year_month_day(date_minor_breaks = duration_days(1))
...
test_that("less common scales have basic support", {
  date <- year_month_day(2019, 1) + duration_months(1:5)
