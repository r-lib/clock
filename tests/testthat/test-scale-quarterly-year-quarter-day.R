...
#   df <- data_frame(
#     x = year_quarter_day(integer(), integer()),
#     y = double()
#   )
#
...
test_that("works with a vector of data", {
  df <- data_frame(
    x = year_quarter_day(2019, 1) + duration_quarters(1:5),
    y = 1:5
  )
...
test_that("works with changing the `date_breaks`", {
  df <- data_frame(
    x = year_quarter_day(2019, 1) + duration_quarters(1:20),
    y = 1:20
  )
  ...
  p <- ggplot2::ggplot(df, ggplot2::aes(x, y)) +
    ...
    scale_x_year_quarter_day(
      date_breaks = duration_quarters(4),
      date_minor_breaks = duration_quarters(2)
    )
...
test_that("can't use invalid `date_breaks` or `date_minor_breaks`", {
  ...
  p <- ggplot2::ggplot(df, ggplot2::aes(x, y)) +
    ggplot2::geom_line() +
    scale_x_year_quarter_day(date_breaks = duration_days(1))
  ...
  p <- ggplot2::ggplot(df, ggplot2::aes(x, y)) +
    ggplot2::geom_line() +
    scale_x_year_quarter_day(date_minor_breaks = duration_days(1))
...
test_that("less common scales have basic support", {
  date <- year_quarter_day(2019, 1) + duration_quarters(1:5)
