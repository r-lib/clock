...
test_that("printing zoned-datetime - extended format", {
  x <- zoned_datetime(1970, 10, 25, 01, 30, zone = "America/New_York")
  expect_snapshot_output(format(x))
  expect_snapshot_output(format(x, format = fmt_zoned_datetime(extended = TRUE)))
  expect_snapshot_output(format(x, format = fmt_zoned_datetime(extended = TRUE), abbreviate_zone = TRUE))
})
