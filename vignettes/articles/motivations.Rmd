...
R has always had strong date and date-time capabilities. The Date, POSIXct, and POSIXlt classes provide a powerful foundation to build on, and the R community has done just that. [lubridate](https://lubridate.tidyverse.org/), [zoo](https://cran.r-project.org/web/packages/zoo/index.html), and [tsibble](https://cran.r-project.org/web/packages/tsibble/index.html) are just a few of the popular packages that have extended R's native date-time capabilities.
...
```{r}
x <- naive_parse("2020-11-01 00:30:00")
x <- add_hours(x, 0:5)
x
```
...
```{r}
sec <- as_sys(year_month_day(
  2019, 2, 3, 1, 30, 35
))

milli <- as_sys(year_month_day(
  2019, 2, 3, 1, 30, 35, 0, 
  subsecond_precision = "millisecond"
))
