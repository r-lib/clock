...
- Pin to the exact end of the current month, including time of day.

- Pin to the exact start of the following month, including time of day.

- Adjust the day of the month to the end of the current month, but keep the original time of day.

- Adjust the day of the month to the start of the following month, but keep the original time of day.

- Return an `NA` because this date doesn't exist.

- Error because this date doesn't exist.

The most common behavior with the year/month/day components is to move to the end of the current month `"2019-02-28"`. By default, civil will also pin the time of day to the very end of the day, `23:59:59`.
...
But there is nothing stopping you from choosing a different behavior. You can adjust the default behavior with `day_nonexistant`.
...
```{r}
add_days(x, 1, dst_nonexistant = "next-shift")
```
...
To resolve this nonexistent date, it seems intuitive to take into account the _direction_ that you are traveling when performing this arithmetic. Rather than moving forward to `03:00:00`, it makes sense to roll back to the previous instant in time of `01:59:59` since that matches the direction of travel you are already moving in.

To handle this, the `dst_nonexistant` argument defaults to `"directional"`, which chooses `"next"` if you are adding time and `"previous"` if you are subtracting it. This hopefully gives the most intuitive result of:
...
```{r}
subtract_days(y, 1, dst_nonexistant = "next")
```
...
Like with `dst_nonexistant`, the direction that you approach from can influence what you expect the result of this arithmetic to be. Let's shift past the fallback time, and then subtract a month:
...
In previous sections, we noted that there are arguments to `day_nonexistant` and `dst_nonexistant` that attempt to retain the sub-daily components of you date-time vector when resolving nonexistant date-times.

For `day_nonexistant`, these are `"end-keep"` and `"start-keep"`.

For `dst_nonexistant`, these are `"next-shift"`, `"previous-shift"`, and `"directional-shift"`.
...
What happens if you add 1 month to this vector, and want to keep the time of day? Note that the second element in the vector will have to consult `day_nonexistant` since `"1970-02-31"` doesn't exist.

```{r}
y <- add_months(x, 1, day_nonexistant = "end-keep")
...
The defaults in civil will always preserve relative ordering. For `day_nonexistant`, it is always safe to use `"end"` and `"start"`. This is why `"end"` pins the time of day to the last possible second in the day.

```{r}
z <- add_months(x, 1, day_nonexistant = "end")
z
...
### Nonexistant times due to daylight savings gaps
...
If we added 1 day to this vector and used `"next-shift"` to keep the time of day by "shifting" forward by the size of the gap, then we'd end up with:

```{r}
y <- add_days(x, 1, dst_nonexistant = "next-shift")
y
...
The default option for `dst_nonexistant` of `"directional"` will preserve relative ordering, as will choosing `"next"` or `"previous"` directly. This is why `"next"` pins the time of day to the first possible second after the gap (and similar reasoning applies for `"previous"`).
...
The issue with adding multiple periods is that `add_days()` and friends will _immediately_ try to fix any daylight savings issues (i.e. with `dst_nonexistant`) after each individual unit is added. This means that result of adding multiple periods can depend on the order in which you apply them.

```{r}
x %>%
  add_days(1) %>% # immediately applies `dst_nonexistent = "next"`
  print() %>%
  add_hours(1)
...
A local date-time is one that isn't associated with any time zones. This frees it from the issues that `dst_nonexistant` and `dst_ambiguous` have to handle.
