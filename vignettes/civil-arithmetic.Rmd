...
add_months(x, 1, day_nonexistent = "end-keep")
```
...
```{r}
add_days(x, 1, dst_nonexistent = "next-shift")
```
...
To handle this, the `dst_nonexistent` argument defaults to `"directional"`, which chooses `"next"` if you are adding time and `"previous"` if you are subtracting it. This hopefully gives the most intuitive result of:
...
```{r}
subtract_days(y, 1, dst_nonexistent = "next")
```
...
For `day_nonexistent`, these are `"end-keep"` and `"start-keep"`.

For `dst_nonexistent`, these are `"next-shift"`, `"previous-shift"`, and `"directional-shift"`.
...
```{r}
y <- add_months(x, 1, day_nonexistent = "end-keep")
...
The defaults in civil will always preserve relative ordering. For `day_nonexistent`, it is always safe to use `"end"` and `"start"`. This is why `"end"` pins the time of day to the last possible second in the day.

```{r}
z <- add_months(x, 1, day_nonexistent = "end")
z
...
```{r}
y <- add_days(x, 1, dst_nonexistent = "next-shift")
y
...
The default option for `dst_nonexistent` of `"directional"` will preserve relative ordering, as will choosing `"next"` or `"previous"` directly. This is why `"next"` pins the time of day to the first possible second after the gap (and similar reasoning applies for `"previous"`).
