...
- [`date_zone()`](https://clock.r-lib.org/reference/date-zone.md) and
  [`date_set_zone()`](https://clock.r-lib.org/reference/date-zone.md)
...
- [`date_seq()`](https://clock.r-lib.org/reference/date_seq.md) and the
  [`seq()`](https://rdrr.io/r/base/seq.html) methods for the calendar,
  time point, and duration types now allow `from > to` when `by > 0`.
  This now results in a size zero result rather than an error, which is
  more in line with
...
- New [`date_seq()`](https://clock.r-lib.org/reference/date_seq.md) for
  generating date and date-time sequences
...
- clock now interprets R’s Date class as *naive-time* rather than
  *sys-time*. This means that it no longer assumes that Date has an
  ...
  - [`date_zone()`](https://clock.r-lib.org/reference/date-zone.md) now
    errors with Date input, as naive-times do not have a specified time
    zone.
...
- New [`as_date()`](https://clock.r-lib.org/reference/as_date.md) and
  ...
  [`as_date()`](https://clock.r-lib.org/reference/as_date.md) does
  conflict with
...
- Added two new convenient helpers
  ...
  - [`date_now()`](https://clock.r-lib.org/reference/date-today.md) for
    getting the current date-time (POSIXct)
